 ...
 * @ingroup ipc_android
 */
// Verbatim from upstream Monado (src/xrt/ipc/android/src/main/java/...) with
// exactly one adaptation below: BuildConfig.SERVICE_ACTION replaced with its
// own literal value. Upstream generates that constant via its own module's
// build.gradle (buildConfigField); here it's compiled as part of server-app
// (namespace org.meumeu.wivrn.server), so unqualified BuildConfig would
...
public class Client implements ServiceConnection {
    ...
    public boolean bind(Context context_, String packageName) {
        ...
        Intent intent = new Intent("org.freedesktop.monado.ipc.CONNECT").setPackage(packageName);
